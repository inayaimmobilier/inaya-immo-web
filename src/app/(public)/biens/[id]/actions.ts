...
import { enregistrerSignalement, type ResultatSignalement } from "@/lib/signalement"
import {
 ...
 *
 * La logique vit dans `@/lib/signalement`, partagée avec l'API de
 * l'application mobile : dupliquée, la liste des motifs aurait divergé au
 * premier ajout.
 */
export async function signalerAnnonce(form: FormData): Promise<ResultatSignalement> {
  const anon = await createClient()
  const { data: { user } } = await anon.auth.getUser()
  return enregistrerSignalement({
